...
import { ArrowLeft, ArrowLeftCircle, ArrowRightCircle } from 'react-bootstrap-icons';
import Image from './Image';

function ImageCarousel({ data, className }) {
  const { images } = data;
  const refs = [];
  ...
  const carousel = images.map((data) => {
    const imgRef = createRef();
    refs.push(imgRef);
    return (
      <div className="image-carousel-item" key={data.url} ref={imgRef}>
        <Image data={data} />
      </div>
    );
  });
      ...
      <div className="d-flex justify-content-between m-1">
        ...
        <div className="flex-grow-1 ps-1 pe-1 text-center d-flex justify-content-start align-items-center">
          <div className="d-flex flex-grow-1 mb-1">
            <div style={{ width: `${((viewIndex + 1) / images.length) * 100}%` }} className="progress-bar progress-bar-dark" />
            <div style={{ width: `${((images.length - (viewIndex + 1)) / images.length) * 100}%` }} className="progress-bar progress-bar-light" />
          </div>
